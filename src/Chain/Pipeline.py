...
async def main_chain(client, file_path: Path, filename: str, fileid: int):
    # try:
        ...
        async with client.session("service") as session:
            ...
            # row.progress = row.progress_done / row.progress_total * 100
            # await session.commit()
            response = await processor_with_embeddings(file_path=file_path, file_type=file_type_response.document_type)


        # async with client.session("service") as session:
        #     row = await session.get(PDF_Queue, fileid)
        #     logger.info("Stage 3. Extract text from PDF")
        #     row.status_description = "Stage 3. Extract text from PDF"
        #     row.progress_done += 1
        #     row.progress = row.progress_done / row.progress_total * 100
        #     await session.commit()
        ...
        # async with client.session("service") as session:
        #     row = await session.get(PDF_Queue, fileid)
        #     row.status_description = "Stage 4. Extract JSON from extracted raw text. Waiting for LLM answer..."
        #     row.progress_done += 1
        #     row.progress = row.progress_done / row.progress_total * 100
        #     await session.commit()
        #     logger.info("Stage 3. Extract JSON from extracted raw text. Waiting for LLM answer...")
